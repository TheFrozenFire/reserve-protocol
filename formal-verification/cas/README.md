# Algebraic verification (PARI/GP) for Reserve smart contracts
...
## Two layers, orthogonal guarantees

The Rocq layer (`../rocq/`) proves logical soundness: under the
simulation's assumptions, the invariant holds for all inputs. The CAS
layer here validates faithfulness: the algebraic claims actually agree
with concrete computation across calibrated inputs.

Both are necessary. A Rocq proof can be impeccable yet verifying the
wrong model. A CAS sweep can catch boundary anomalies before any
proof is attempted. The `_xcheck.v` and `_witnesses.v` files under
`../rocq/proofs/` pin specific CAS-found values as `vm_compute;
reflexivity` theorems, so numerical drift between the layers fails
the build.
